...
# Fungsi: minta port dari user (untuk panel)
get_port() {
    while true; do
...
uninstall_ptero() {
    ...
    if [[ -z "$INSTANCE" ]]; then
        ...
        systemctl stop nginx php*-fpm mariadb redis-server pteroq*.service wings* 2>/dev/null || true
        ...
        echo -e "${RED}PERINGATAN: Ini akan menghapus instance Pterodactyl $INSTANCE, Wings terkait, dan file terkait.${NC}"
        read -rp "Lanjutkan? (y/N): " ans
        [[ "$ans" != "y" && "$ans" != "Y" ]] && { echo -e "${YELLOW}Dibatalkan.${NC}"; exit 0; }
        ...
        rm -rf /var/www/pterodactyl-$INSTANCE /etc/pterodactyl-$INSTANCE /etc/nginx/sites-{available,enabled}/pterodactyl-$INSTANCE.conf
