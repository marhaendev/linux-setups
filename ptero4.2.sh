...
# Fungsi: uninstall Wings
uninstall_wings() {
    echo -e "${RED}PERINGATAN: Ini akan menghapus Pterodactyl Wings, file konfigurasi, dan aturan firewall terkait.${NC}"
    read -rp "Lanjutkan? (y/N): " ans
    [[ "$ans" != "y" && "$ans" != "Y" ]] && { echo -e "${YELLOW}Dibatalkan.${NC}"; exit 0; }
    ...
    if command -v ufw >/dev/null 2>&1; then
        ...
        ufw delete allow 2022 >/dev/null 2>&1
        echo -e "${GREEN}✅ Port Wings 8080 dan 2022 telah dihapus dari firewall.${NC}"
    fi
    echo -e "${GREEN}=== UNINSTALL WINGS SELESAI ===${NC}"
}
...
echo "881) Uninstall Wings"
echo "882) Install Wings"
echo "888) Konfigurasi token Wings untuk instance tertentu"
read -rp "Pilih opsi [0-6,12-13,881-882,888]: " choice
