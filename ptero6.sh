...
# Fungsi: uninstall bersih untuk instance tertentu atau semua
uninstall_ptero() {
    ...
    if [[ -z "$INSTANCE" ]]; then
        ...
        apt-get purge -y nginx* mariadb-* mysql-* redis-server redis-tools php* composer nodejs npm certbot 2>/dev/null || true
        apt-get autoremove -y --purge
        apt-get clean
        rm -rf /var/www/pterodactyl* /etc/nginx /etc/php /etc/mysql /var/lib/mysql /var/lib/redis /etc/redis /var/log/redis /var/run/redis
        crontab -l 2>/dev/null | grep -v "pterodactyl" | crontab - 2>/dev/null || true
        rm -f /etc/systemd/system/pteroq*.service
        systemctl daemon-reload
        mysql -u root -e "DROP DATABASE IF EXISTS panel_*;" 2>/dev/null || true
...
install_ptero() {
    ...
    local golang="$6"
    check_dependencies
    check_redis || { echo -e "${RED}❌ Redis tidak tersedia. Instalasi dihentikan.${NC}"; exit 1; }
    ...
    systemctl enable nginx php8.2-fpm mariadb redis-server 2>/dev/null || { echo -e "${RED}❌ Gagal mengaktifkan layanan. Periksa status layanan.${NC}"; exit 1; }
    systemctl start nginx php8.2-fpm mariadb redis-server 2>/dev/null || { echo -e "${RED}❌ Gagal memulai layanan. Periksa status layanan.${NC}"; exit 1; }
    mysql <<SQL
...
SQL
    ...
    mysql -u "$DB_USER" -p"$DB_PASS" -h 127.0.0.1 -e "SELECT 1" 2>/dev/null || {
        echo -e "${RED}❌ Gagal koneksi ke database dengan user $DB_USER. Mencoba memperbaiki...${NC}"
        mysql -u root <<SQL
...
SQL
        mysql -u "$DB_USER" -p"$DB_PASS" -h 127.0.0.1 -e "SELECT 1" 2>/dev/null || {
            echo -e "${RED}❌ Gagal koneksi ke database setelah perbaikan. Periksa kredensial di .env.${NC}"
            exit 1
        }
    }
...
ENV
    ...
    php composer.phar install --no-dev --optimize-autoloader || {
        ...
        php composer.phar install --no-dev --optimize-autoloader || {
            echo -e "${RED}❌ Gagal menginstal dependensi Composer.${NC}"
            exit 1
        }
    }
    php artisan optimize:clear
    php artisan migrate --seed --force || {
        echo -e "${RED}❌ Gagal menjalankan migration. Periksa log database.${NC}"
        exit 1
    }
...
NGINX
    ...
    nginx -t || { echo -e "${RED}❌ Konfigurasi Nginx gagal. Periksa /etc/nginx/sites-available/pterodactyl-$instance.conf.${NC}"; exit 1; }
...
SQL
    ...
    # Verifikasi layanan
    systemctl restart nginx php8.2-fpm mariadb redis-server pteroq-$instance.service
